...
def validate_inputs_early_failure(inputs: WritingFlowInputs) -> Dict[str, Any]:
    ...
    valid_content_types = ["STANDALONE", "SERIES"]
    if inputs.content_type not in valid_content_types:
        validation_results["critical_errors"].append(f"Invalid content type. Must be one of: {valid_content_types}")
    ...
    try:
        ...
        if not content_path.exists():
            validation_results["critical_errors"].append(f"File path does not exist: {inputs.file_path}")
        elif not (content_path.is_file() or content_path.is_dir()):
            validation_results["critical_errors"].append(f"File path is not a file or directory: {inputs.file_path}")
    ...
    try:
        # Check if we have write permissions in the file path directory
        ...
        try:
            ...
            raise RuntimeError(f"No write permissions in directory: {parent_dir}")
    except Exception as e:
        validation_results["prerequisites_met"] = False
        raise RuntimeError(f"System prerequisites check failed: {str(e)}")
