...
def check_duplicates(rules: List[Dict]) -> Tuple[int, List[str]]:
    ...
    for idx, r in enumerate(rules):
        ...
        if key in seen_content:
            dups.append(f"duplicate rule for platform at {idx} and {seen_content[key]}")
        else:
...
def main():
    parser = argparse.ArgumentParser(description="Validate Publisher Platform transformed rules JSON")
    parser.add_argument("--path", default=os.path.abspath(os.path.join(os.path.dirname(__file__), "output", "publisher_platform_chromadb_rules.json")))
